...
Contact(s):  Minna Win
Abstract:
History Log:  Initial version (supports CNT, CTC, CTS, and SL1L2 line types for point
stat .stat file)
...
class WriteStatAscii:
    ...
    def write_stat_ascii(self, stat_data: pd.DataFrame, parms: dict):
        """ write MET stat files (.stat) to an ASCII file with stat_name, stat_value,
            stat_bcl, stat_bcu, stat_ncl, and stat_ncu columns,
            converting the original data file from  wide form to long form.
            ...
            Returns:  None, write an output ASCII file associated with the original
            MET .stat file with statistics
                      information aggregated into these six columns: stat_name,
    ...
    def process_cnt(self, stat_data: pd.DataFrame) -> pd.DataFrame:
        ...
        confidence_level_columns_renamed: List[
            str] = self.rename_confidence_level_columns(cnt_df.columns.tolist())
        ...
        # STAT_MAE, etc.)
        stat_confidence_level_columns_renamed = self.rename_statistics_columns(cnt_df,
                                                                               cn.CNT_STATISTICS_HEADERS)
    ...
    def process_ctc(self, stat_data: pd.DataFrame) -> pd.DataFrame:
        ...
        linetype_data = ctc_df.melt(id_vars=id_vars_list,
    ...
    def process_cts(self, stat_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # (i.e. from FBAR_BCU to BCU_FBAR to be able to use the pandas wide_to_long).
        confidence_level_columns_renamed: List[
            str] = self.rename_confidence_level_columns(cts_df.columns.tolist())
        cts_df.columns: List[str] = confidence_level_columns_renamed

        # Rename the statistics columns (ie. FBAR, MAE, FSTDEV, etc. to STAT_FBAR,
        # STAT_MAE, etc.)
        stat_confidence_level_columns_renamed = self.rename_statistics_columns(cts_df,
                                                                               cn.CTS_STATS_ONLY_HEADERS)
    ...
    def process_sl1l2(self, stat_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Columns we don't want to stack (i.e. treat these columns as a multi index)
        id_vars_list = ['Idx'] + cn.LC_COMMON_STAT_HEADER + ['total']
        reshaped = sl1l2_df.melt(id_vars=id_vars_list,
                                 value_vars=cn.SL1L2_STATISTICS_HEADERS,
                                 var_name='stat_name',
                                 value_name='stat_value').sort_values('Idx')
        # Original way to sort using new index
                                 # value_name='stat_value').sort_values('Idx')
...
def main():
    ...
    # Get all the filenames in the data_dir specified above (and derived from
    # the base of the stat_input setting value.
    load_files = xml_loadfile_obj.filenames_from_template(
        parms['data_dir'], {})
    print(f"All files that will be loaded: {load_files}")
