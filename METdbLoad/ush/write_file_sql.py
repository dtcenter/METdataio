...
class WriteFileSql:
    ...
    def __init__(self, logger=None):
        try:
            ...
            if logger is None:
                print(
                    "*** %s occurred while initializing class WriteFileSql ***", sys.exc_info()[0])
            else:
                self.logger = logger
                self.logger.error(
                    "*** %s occurred while initializing class WriteFileSql ***", sys.exc_info()[0])
            sys.exit("*** Error initializing class WriteFileSql")
    ...
    def write_file_sql(self, load_flags, data_files, stat_data, mode_cts_data,
        ...
        try:
            ...
            try:
                ...
                next_file_id = self.sql_met.get_next_id(
                    CN.DATA_FILE, CN.DATA_FILE_ID, sql_cur, self.logger)
                if next_file_id == 0:
                    next_file_id = 1
                ...
                for row_num, file_line in data_files.iterrows():
                    ...
                    sql_cur.execute(
                        CN.Q_FILE, [file_line[CN.FILEPATH], file_line[CN.FILENAME]])
                    ...
                    if sql_cur.rowcount > 0:
                        ...
                        if not load_flags['force_dup_file']:
                            ...
                            data_files.loc[data_files.index[row_num],
                ...
                index_names = data_files[data_files.data_file_id ==
                                         CN.NO_KEY].index
                ...
                if not data_files.empty:
                    ...
                    new_files = data_files[data_files[CN.DATA_FILE_ID]
                                           >= next_file_id]
            ...
            self.logger.error(
                "*** %s occurred in write_file_sql function ***", sys.exc_info()[0])
            sys.exit("*** Error in write_file_sql function")
    ...
    def write_metadata_sql(self, load_flags, data_files, group, description,
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    "*** %s in write_metadata_sql ***", sys.exc_info()[0])
                sys.exit("*** Error writing metadata_SQL")
