...
class WriteStatSql:
    ...
    @staticmethod
    def write_stat_data(load_flags, stat_data, tmp_dir, sql_cur, local_infile, logger):
        ...
        try:
            ...
            logger.debug("[--- Start write_stat_data ---]")
            ...
            try:
                ...
                next_header_id = sql_met.get_next_id(
                    CN.STAT_HEADER, CN.STAT_HEADER_ID, sql_cur, logger)
                ...
                if load_flags["stat_header_db_check"]:
                    ...
                    for row_num, data_line in stat_headers.iterrows():
                        sql_cur.execute(
                            CN.Q_HEADER, data_line.values[1:-1].tolist())
                        result = sql_cur.fetchone()

                        # If you find a match, put the key into the stat_headers dataframe
                        if sql_cur.rowcount > 0:
                            stat_headers.loc[stat_headers.index[row_num],
                                             CN.STAT_HEADER_ID] = result[0]
                        else:
                ...
                new_headers = stat_headers[stat_headers[CN.STAT_HEADER_ID] > (
                    next_header_id - 1)]
                logger.info("New headers: %s rows",
                            str(len(new_headers.index)))
            ...
            try:
                ...
                for line_type in line_types:
                    ...
                    line_table = CN.LINE_TABLES[CN.UC_LINE_TYPES.index(
                        line_type)]

                    # get the line data of just this type and re-index
                    line_data = stat_data[stat_data[CN.LINE_TYPE]
                                          == line_type].copy()
                    line_data = line_data.reset_index(drop=True)
                    logger.info("%s: %s rows", line_type,
                    ...
                    if line_type in CN.VAR_LINE_TYPES:
                        ...
                        next_line_id = \
                            sql_met.get_next_id(
                                line_table, CN.LINE_DATA_ID, sql_cur, logger)
                        ...
                        line_data[CN.LINE_DATA_ID] = line_data.index + \
                            next_line_id

                        # index of the first column of the repeating variables
                        var_index = line_data.columns.get_loc(
                            CN.LINE_VAR_COUNTER[line_type]) + 1
                        ...
                        for row_num, file_line in line_data.iterrows():
                            ...
                            var_count = int(
                                file_line[CN.LINE_VAR_COUNTER[line_type]])
                            ...
                            if repeat_width > (len(file_line) - var_index):
                                file_name = sql_met.get_file_name(
                                    file_line[CN.DATA_FILE_ID], sql_cur, logger)
                                logger.error('*** Variable length record from file %s line %s '
                                ...
                                var_data = \
                                    pd.DataFrame(list_var_data.values.reshape(
                                        var_count, var_repeats))
                                ...
                                var_data.insert(
                                    0, CN.LINE_DATA_ID, file_line[CN.LINE_DATA_ID])
                                var_data.insert(
                                    1, 'i_value', var_data.index + 1)

                                # MCTC has i and j counters where j increments faster
                                if line_type == CN.MCTC:
                                    var_data.loc[:, 'i_value'] = \
                                        np.repeat(
                                            np.array(range(1, basic_count + 1)), basic_count)
                                    j_indices = np.resize(
                                        range(1, basic_count + 1), var_count)
                                    ...
                                    line_data.iloc[row_num, var_index] = \
                                        line_data.iloc[row_num,
                                                       var_index + repeat_width]
                        ...
                        if list_var:
                            all_var = pd.concat(
                                list_var, ignore_index=True, sort=False)
                            list_var = []

                        if line_type == CN.RHIST:
                            # copy the RHIST columns and create ECNT lines from them
                            line_data2 = line_data[line_data[CN.VERSION].isin(
                                CN.RHIST_OLD)].copy()
                            if not line_data2.empty:
                                ...
                                sql_met.write_to_sql(line_data2, CN.LINE_DATA_COLS[CN.ECNT],
                                                     CN.LINE_TABLES[CN.UC_LINE_TYPES.index(
                                                         CN.ECNT)],
                ...
                if CN.FCST_PERC in stat_data:
                    if stat_data[CN.FCST_PERC].ne(CN.MV_NOTAV).any():
                        ...
                        sql_met.write_to_sql(line_data2, CN.LINE_DATA_COLS[CN.PERC],
                                             CN.LINE_TABLES[CN.UC_LINE_TYPES.index(
                                                 CN.PERC)],
                                             CN.LINE_DATA_Q[CN.PERC], tmp_dir, sql_cur, local_infile, logger)
                        line_data2 = line_data2.iloc[0:0]
            ...
            logger.info("    >>> Write time Stat: %s", str(write_time))
