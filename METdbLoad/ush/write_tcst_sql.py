    ...
    def write_tcst_data(load_flags, tcst_data, tmp_dir, sql_cur, local_infile, logger):
        ...
        try:
            ...
            try:
                ...
                tcst_headers.drop_duplicates(
                    CN.TCST_HEADER_KEYS[1:], keep='first', inplace=True)
                ...
                next_header_id = sql_met.get_next_id(
                    CN.TCST_HEADER, CN.TCST_HEADER_ID, sql_cur, logger)
                ...
                if load_flags["tcst_header_db_check"]:
                    ...
                    for row_num, data_line in tcst_headers.iterrows():
                        sql_cur.execute(CN.Q_HEADER_TCST,
                                        data_line.values[1:-1].tolist())
                        result = sql_cur.fetchone()

                        # If you find a match, put the key into the tcst_headers dataframe
                        if sql_cur.rowcount > 0:
                            tcst_headers.loc[tcst_headers.index[row_num],
                                             CN.TCST_HEADER_ID] = result[0]
                        else:
                ...
                new_headers = tcst_headers[tcst_headers[CN.TCST_HEADER_ID] > (
                    next_header_id - 1)]
                ...
                logger.error(
                    "*** %s in write_tcst_data write tcst headers ***", sys.exc_info()[0])
                sys.exit("*** Error writing tcst SQL headers")
            ...
            try:
                ...
                for line_type in line_types:
                    ...
                    line_table = CN.LINE_TABLES_TCST[CN.UC_LINE_TYPES_TCST.index(
                        line_type)]

                    # get the line data of just this type and re-index
                    line_data = tcst_data[tcst_data[CN.LINE_TYPE]
                                          == line_type].copy()
                    ...
                    if line_type == CN.TCMPR:
                        line_data['64'] = line_data['64'].replace(
                            CN.NOTAV, 'X')
                        line_data['65'] = line_data['65'].replace(
                            CN.NOTAV, 'X')
                    ...
                    if line_type in CN.VAR_LINE_TYPES_TCST:
                        ...
                        for row_num, file_line in line_data.iterrows():
                            ...
                            var_data.insert(0, CN.LINE_DATA_ID,
                                            file_line[CN.LINE_DATA_ID])
                ...
                logger.error(
                    "*** %s in write_tcst_data write line data ***", sys.exc_info()[0])
                sys.exit("*** Error writing tcst SQL line data")
            ...
            self.logger.error(
                "*** %s occurred in write_tcst_data function ***", sys.exc_info()[0])
            sys.exit("*** Error in write_tcst_data function")
