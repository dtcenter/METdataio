...
class ReadDataFiles:
        ...
        try:
            ...
            try:
                ...
                self.data_files[CN.DATA_FILE_LU_ID] = \
                    np.vectorize(self.get_lookup)(
                        self.data_files[CN.FULL_FILE])
                ...
                self.data_files[CN.FILEPATH] = \
                    self.data_files[CN.FULL_FILE].str.rpartition(CN.FWD_SLASH)[
                    0]
                ...
                for row in self.data_files.itertuples(name=None):
                    ...
                    try:
                        ...
                        if file_and_path.is_file():
                            ...
                            if lu_id == CN.STAT:
                                ...
                                try:
                                    ...
                                    any good files in this batch.
                                    """
                                    self.logger.warning(
                                        f"!!! Stat file {filename} has no columns")
                                    continue
                            ...
                            #
                                ...
                                try:
                                    ...
                                    """
                                    self.logger.warning(
                                        f"!!! Mode file {filename} has no columns")
                                    continue
                            ...
                            #
                                ...
                                try:
                                    ...
                                    """
                                    self.logger.warning(
                                        f"!!! TCST file {filename} has no columns")
                                    continue
                            ...
                            #
                                ...
                                try:
                                    ...
                                    """
                                    self.logger.warning(
                                        f"!!! MTD file {filename} has no columns")
                                    continue
                                ...
                                if CN.FCST_T_BEG not in hdr_names:
                                    mtd_file.insert(
                                        8, CN.FCST_T_BEG, CN.MV_NULL)
                                if CN.FCST_T_END not in hdr_names:
                                    mtd_file.insert(
                                        9, CN.FCST_T_END, CN.MV_NULL)
                                if CN.OBS_T_BEG not in hdr_names:
                                    mtd_file.insert(
                                        12, CN.OBS_T_BEG, CN.MV_NULL)
                                if CN.OBS_T_END not in hdr_names:
                                    mtd_file.insert(
                                        13, CN.OBS_T_END, CN.MV_NULL)
                                ...
                                """
                                We do NOT want to exit here. One bad file does not mean that we should not load
                                any good files in this batch.
                                """
                                self.logger.warning(
                                    f"!!! File type of {filename} not valid")
                            ...
                            if not one_file.empty:
                                ...
                                self.logger.debug(
                                    f"Lines in {filename}: {str(len(one_file.index))}")
                                ...
                                self.logger.debug(
                                    f"Lines in {filename}, {str(len(vsdb_file.index))}")
                                vsdb_file = vsdb_file.iloc[0:0]
                            elif not mode_file.empty:
                                self.logger.debug(
                                    f"Lines in {filename},{str(len(mode_file.index))}")
                                ...
                                self.logger.debug(
                                    f"Lines in {filename}, {str(len(mtd_file.index))}")
                            ...
                            """
                            We do NOT want to exit here. Even if a process deleted one of the files in the batch, we
                            should still check to see if the others are there.
                            """
                        ...
                        """
                        self.logger.warning(
                            f"*** Error produced by one file: {str(e)}. Ignoring this bad file and continuing on ***")
            ...
            try:
                ...
                if list_frames:
                    ...
                    if not all_stat['1'].dtypes == 'float':
                        all_stat.loc[all_stat['1'] ==
                                     CN.NOTAV, '1'] = CN.MV_NOTAV
                    ...
                    if not alpha_lines.empty:
                        self.logger.warning(
                            f"!!! ALPHA line_type has ALPHA value of NA:\r\n {str(alpha_lines)}")
                    ...
                    if not non_alpha_lines.empty:
                        self.logger.warning(
                            f"!!! non-ALPHA line_type has ALPHA float value:\r\n  {str(non_alpha_lines)}")
            ...
            try:
                ...
                if list_tcst:
                    all_tcst = pd.concat(
                        list_tcst, ignore_index=True, sort=False)
                    list_tcst = []
            ...
            try:
                ...
                if list_vsdb:
                    all_vsdb = pd.concat(
                        list_vsdb, ignore_index=True, sort=False)
                    list_vsdb = []
            ...
            try:
                if list_cts:
                    all_cts = pd.concat(
                        list_cts, ignore_index=True, sort=False)
                    list_cts = []
                ...
                if list_obj:
                    ...
                    all_obj = pd.concat(
                        list_obj, ignore_index=True, sort=False)
                    list_obj = []
                ...
                self.logger.error(
                    f"*** Error transforming cts or obj data prior to load: {sys.exc_info()[0]}*** ")
                sys.exit(
                    "*** Error transforming cts or obj data prior to load")

            try:
                if not all_stat.empty:
                    self.logger.debug(
                        f"Shape of all_stat before transforms: {str(all_stat.shape)}")
                    ...
                    if not invalid_line_indexes.empty:
                        ...
                        self.logger.warning(
                            f"line types: {str(all_stat.iloc[invalid_line_indexes].line_type)}")

                        all_stat.drop(invalid_line_indexes,
                                      axis=0, inplace=True)
                    ...
                    self.logger.debug(
                        f"Shape of all_stat after transforms: {str(all_stat.shape)}")
                ...
                self.logger.error(
                    f"*** Error transforming data prior to load: {sys.exc_info()[0]}*** ")
                sys.exit(
                    "*** Error transforming data prior to load")

            try:
                if not all_tcst.empty:

                    self.logger.debug(
                        f"Shape of all_tcst before transforms:  {str(all_tcst.shape)}")
                    ...
                    if not invalid_line_indexes.empty:
                        ...
                        self.logger.warning("!!! Warning, invalid line_types:")
                        self.logger.warning(
                            f"line types: {str(all_tcst.iloc[invalid_line_indexes].line_types)}")

                        all_tcst.drop(invalid_line_indexes,
                                      axis=0, inplace=True)
                ...
                self.logger.error(
                    f"*** Error transforming tcst data prior to load: {sys.exc_info()[0]}*** ")
                sys.exit(
                    "*** Error transforming tcst data prior to load")
            ...
            try:
                ...
                self.logger.error(
                    f"*** Error transforming 2d obj data prior to load: {sys.exc_info()[0]}*** ")
                sys.exit(
                    "*** Error transforming 2d obj data prior to load")
            ...
            try:
                ...
                self.logger.error(
                    f"*** Error transforming single obj data prior to load: {sys.exc_info()[0]}*** ")
                sys.exit(
                    "*** Error transforming single obj data prior to load")

            try:
                if list_pair:
                    all_pair = pd.concat(
                        list_pair, ignore_index=True, sort=False)
                    list_pair = []
    ...
    def read_stat(self, filename, hdr_names):
        ...
        try:
            ...
            """
            self.logger.warning(
                f"!!! Stat file {filename} has no data after headers")
            return stat_file
    ...
    def read_tcst(self, filename, hdr_names):
        ...
        try:
            ...
            """
            self.logger.warning(
                f"!!! Tcst file {filename} has no data after headers")
            return stat_file
    ...
    def read_mode(self, filename, hdr_names):
        ...
        try:
            ...
            """
            self.logger.warning(
                f"!!! Mode or MTD file {filename} has no data after headers")
            return stat_file
