    ...
    def write_mtd_data(load_flags, m_2d_data, m_3d_single_data, m_3d_pair_data,
        ...
        try:
            ...
            try:
                ...
                mtd_headers = mtd_headers.drop_duplicates(
                    CN.MTD_2D_HEADER_KEYS, keep='first')
                ...
                next_header_id = sql_met.get_next_id(
                    CN.MTD_HEADER, CN.MTD_HEADER_ID, sql_cur, logger)
                ...
                if load_flags["mtd_header_db_check"]:
                    ...
                    for row_num, data_line in mtd_headers.iterrows():
                        data_line[CN.FCST_VALID] = \
                            data_line[CN.FCST_VALID].strftime(
                                "%Y-%m-%d %H:%M:%S")
                        data_line[CN.FCST_INIT] = data_line[CN.FCST_INIT].strftime(
                            "%Y-%m-%d %H:%M:%S")
                        if data_line[CN.OBS_VALID] != CN.MV_NULL:
                            data_line[CN.OBS_VALID] = \
                                data_line[CN.OBS_VALID].strftime(
                                    "%Y-%m-%d %H:%M:%S")
                        if CN.MV_NULL not in data_line.values[4:-1].tolist():
                            sql_cur.execute(
                                CN.Q_MTDHEADER, data_line.values[4:-1].tolist())
                        else:
                        ...
                        if sql_cur.rowcount > 0:
                            mtd_headers.loc[mtd_headers.index[row_num],
                                            CN.MTD_HEADER_ID] = result[0]
                ...
                new_headers = mtd_headers[mtd_headers[CN.MTD_HEADER_ID] > (
                    next_header_id - 1)]
                new_headers.obs_valid = pd.to_datetime(
                    new_headers.obs_valid, errors='coerce')
                logger.info("New MTD headers: %s rows",
                ...
                if not new_headers.empty:
                    ...
                    if new_headers[CN.REVISION_ID].ne(CN.MV_NULL).any():
                        ...
                        next_rev_id = sql_met.get_next_id(
                            CN.MTD_HEADER, CN.REVISION_ID, sql_cur, logger)
                ...
                mtd_headers.obs_valid = pd.to_datetime(
                    mtd_headers.obs_valid, errors='coerce')

            except (RuntimeError, TypeError, NameError, KeyError):
                logger.error(
                    "*** %s in write_mtd_sql write MTD headers ***", sys.exc_info()[0])
                sys.exit("*** Error writing MTD SQL headers")
            ...
            try:
                ...
                if not m_2d_data.empty:
                    ...
                    if ((~m_2d_data.object_cat.str.contains(CN.U_SCORE)).sum() > 0 and
                        ...
                        m_2d_data.loc[~m_2d_data.object_cat.str.contains(CN.U_SCORE) &
                                      ~m_2d_data.object_cat.str.contains(
                                          CN.T_ZERO),
                                      CN.MATCHED_FLAG] = 1
                ...
                if not m_3d_single_data.empty:
                    # make sure type of columns is consistent between headers and line data
                    m_3d_single_data.fcst_lead = m_3d_single_data.fcst_lead.astype(
                        'int64')
                    m_3d_single_data.obs_lead = m_3d_single_data.obs_lead.astype(
                        'int64')
                    ...
                    if (~m_3d_single_data.object_cat.str.contains(CN.U_SCORE)).sum() > 0:
                        if (~m_3d_single_data.object_cat.str.contains(CN.T_ZERO)).sum() > 0:
                            m_3d_single_data.loc[~m_3d_single_data.object_cat.str.contains(CN.U_SCORE) &
                                                 ~m_3d_single_data.object_cat.str.contains(
                                                     CN.T_ZERO),
                                                 CN.MATCHED_FLAG] = 1
                ...
                if not m_3d_pair_data.empty:
                    # make sure type of columns is consistent between headers and line data
                    m_3d_pair_data.fcst_lead = m_3d_pair_data.fcst_lead.astype(
                        'int64')
                    m_3d_pair_data.obs_lead = m_3d_pair_data.obs_lead.astype(
                        'int64')
                ...
                logger.error(
                    "*** %s in write_mtd_sql write line data ***", sys.exc_info()[0])
                sys.exit("*** Error writing MTD SQL line data")
            ...
            self.logger.error(
                "*** %s occurred in write_mtd_data function ***", sys.exc_info()[0])
            sys.exit("*** Error in write_mtd_data function")
