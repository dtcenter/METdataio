...
Use the **MET stat-analysis** tool to calculate the aggregation statistics as input to the reformatter (this is only needed
if aggregation statistics are to be plotted).  Otherwise, the .stat files from the MET point-stat, grid-stat, and ensemble-stat
tools can be used as input to the reformatter.
